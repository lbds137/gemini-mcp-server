    ...
    def _process_request(self, request_str: str) -> Optional[dict]:
        ...
        try:
            ...
            try:
                ...
                return JsonRpcResponse(
                    error=JsonRpcError(ERROR_INVALID_REQUEST, str(e)).to_dict(),
                    id=request_id
                ).to_dict()
...
class ResponseCache:
    ...
    def set(self, tool_name: str, parameters: Dict[str, Any],
            response: str, ttl_seconds: Optional[int] = None) -> None:
        """Cache a response."""
        key = self._make_key(tool_name, parameters)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self._cache:
            ages = [(datetime.now() - e.created_at).total_seconds()
...
class ConversationMemory:
    ...
    def add_turn(self, tool: str, input_params: Dict[str, Any],
                 output: str, metadata: Optional[Dict[str, Any]] = None) -> None:
...
class ToolRegistry:
    ...
    def discover_tools(self) -> None:
        ...
        if self._tools:
            logger.info(
                f"Tools already discovered ({len(self._tools)} tools), skipping autodiscovery"
            )
            return

        # For the bundled version, use autodiscovery to find all MCPTool subclasses
        # that are defined in the current module
        import sys
        current_module = sys.modules[__name__]

        logger.info("Auto-discovering MCP tools in bundled server...")
        ...
        logger.info(f"Autodiscovery complete: {len(self._tools)} tools registered")

    def _register_builtin_tools(self) -> None:
        """Legacy method - now handled by discover_tools()."""
        pass
...
class ConversationOrchestrator:
    ...
    def __init__(self, tool_registry: ToolRegistry, model_manager: DualModelManager,
                 memory: ConversationMemory, cache: ResponseCache):
    ...
    async def execute_tool(self, tool_name: str, parameters: Dict[str, Any],
                          request_id: Optional[str] = None) -> ToolOutput:
